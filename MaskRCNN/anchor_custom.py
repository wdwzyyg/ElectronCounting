...
class AnchorGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.stride_multiplier = stride_multiplier
        self.sizes = sizes
    ...
    def grid_anchors(self, grid_sizes: List[List[int]], strides: List[List[Tensor]]) -> List[Tensor]:
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            # shifts_x = shifts_x[shifts_x < grid_width]
            # shifts_y = shifts_y[shifts_y < grid_height]
    ...
    def forward(self, image_list: ImageList, feature_maps: List[Tensor]) -> List[Tensor]:
        ...
        strides = [
            [   # sparse the grid anchors by 3.
                torch.empty((), dtype=torch.int64, device=device).fill_(image_size[0] // g[0] * self.stride_multiplier),
                torch.empty((), dtype=torch.int64, device=device).fill_(image_size[1] // g[1] * self.stride_multiplier),
            ]
            for g in grid_sizes
        ]
        self.set_cell_anchors(dtype, device)
        # print('grid_sizes', len(grid_sizes), 'strides', len(strides), 'cell', len(self.cell_anchors))
