...
def faster_rcnn_2conv(pretrained, num_classes, weights_path, setting_dict):
    ...
    anchor_generator = AnchorGenerator(sizes=((1, 2, 4),), aspect_ratios=((0.5, 1.0, 2.0),))
    box_roi_pool = MultiScaleRoIAlign(featmap_names=["0", "1"], output_size=7, sampling_ratio=2)
    ...
    model = torchvision.models.detection.FasterRCNN(backbone=backbone, num_classes=num_classes,
                                                    transform=transform, anchor_generator=anchor_generator,
                                                    box_head=box_head,
                                                    **setting_dict)
